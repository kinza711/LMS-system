...
export const postAnnouncement = async (req, res) => {
  try {
    ...
    res.status(500).json({
      message: "server error Announcement not posted",
      error: err.message,
    });
  }
};
...
export const deleteAnnouncement = async (req, res) => {
  try {
    ...
    res.status(500).json({
      message: "server error post not deleted",
      error: err.message,
    });
  }
};
...
export const updateAnnouncement = async (req, res) => {
  try {
    ...
    const { id } = req.params;
    const updatepost = await Announcement.findByIdAndUpdate(
      { _id: id },
      req.body,
    );
    res.status(200).json({
    ...
    res.status(500).json({
      message: "server error post not updated",
      error: err.message,
    });
  }
};
